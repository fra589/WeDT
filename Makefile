...
PROJECT=WeDT
RACINE=$(notdir $(shell pwd))

help:
	@cat <<SHELL=/bin/sh
	#-----------------------------------------------------------------------------
	# Root dir of $(PROJECT) project = $$HOME/src/$(PROJECT)
	# Root dir of $(PROJECT) project = $$HOME/src/$(RACINE)
	# make should be executed from this dir:
	# cd $$HOME/src/$(RACINE)
	# Then:
	# make firmware 
	#   Compile the firmware to:
	#   $$HOME/src/$(RACINE)/$(PROJECT)/build/$(PROJECT).ino.bin
	# make littlefs 
	#   Create the application filesystem to
	#   $$HOME/src/$(RACINE)/$(PROJECT)/build/$(PROJECT).littlefs.bin
	# make flash_firmware
	...
	# make clean
	#   Erase the $$HOME/src/$(RACINE)/$(PROJECT)/build directory
	#-----------------------------------------------------------------------------

SHELL=/bin/sh

# Version définie dans $(RACINE)/$(PROJECT).h
VERSION_MAJOR:=$$(grep "define APP_VERSION_MAJOR" $(RACINE)/$(PROJECT).h | awk '{print $$3}' | sed -e 's/"//g')
VERSION_MINOR:=$$(grep "define APP_VERSION_MINOR" $(RACINE)/$(PROJECT).h | awk '{print $$3}' | sed -e 's/"//g')
VERSION_DATE:=$$(grep "define APP_VERSION_DATE" $(RACINE)/$(PROJECT).h | awk '{print $$3}' | sed -e 's/"//g')
...
ARDUINO_BUILD=--pref build.path=${HOME}/src/$(RACINE)/$(PROJECT)/build
...
flash_firmware: firmware
	...
	$(ESPTOOL) --chip $(CHIP) --port $(PORT) --baud $(BAUD) write_flash --flash_size detect --erase-all 0x0 $(FIRMWARE)
